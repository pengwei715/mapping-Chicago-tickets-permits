...
# pylint: disable-msg= too-many-locals, redefined-outer-name
...
for data in ("small", "full"):
    for i in range(1, 4):
        lst = ["company", "race", "gender", "job_category"]
        for levels in permutations(lst, i):
            s = "{}-{}".format(data, ",".join(levels))
            param = pytest.param(data, levels, id=s)
...
def datasets():
    ...
    files_msg = ("Could not find the test files for Task {}. "
                 "Did you remember to run get_files.sh?")
    # Make sure the test files are there
    task1_files = glob.glob("data/tree-*.json")
    if len(task1_files) == 0:
        pytest.fail(files_msg.format("1"))

    task2_files = glob.glob("data/pruned-*.json")
    if len(task2_files) == 0:
        pytest.fail(files_msg.format("2"))

    task3_files = glob.glob("data/rectangles-*.json")
    if len(task3_files) == 0:
        pytest.fail(files_msg.format("3"))

    return d


def compare_trees(tree, expected_tree, error_prefix,
                  compare_counts=False, compare_verbose_labels=False):
    '''
    Compare two tree and generate an error, if they are not the same.
    '''
    ...
    node_error_prefix = error_prefix + \
        "Checking a node with label='{}', count={}, verbose_label={}\n"
    node_error_prefix = node_error_prefix.format(tree.label, tree.count, vl)

    err_msg = node_error_prefix + "Node has incorrect {}. " \
        "Got {}, expected {}"

    if compare_counts:
        assert tree.count == expected_tree["count"], \
            err_msg.format("count", tree.count, expected_tree["count"])

    if compare_verbose_labels:
        assert tree.verbose_label == expected_tree["verbose_label"], \
            err_msg.format("verbose label", vl, expected_tree["verbose_label"])

    children = tree.children
    expected_children = expected_tree["children"]

    if expected_children is None:
        err_msg = node_error_prefix + \
            "Expected node to have no children, but it has children."
        assert children is None or children == [], err_msg
    else:
        for c in children:
            err_msg = node_error_prefix + \
                "Node has a child that is not a Tree: {}".format(c)
            assert isinstance(c, tr.Tree), err_msg

        sorted_children = sorted(children, key=operator.attrgetter("label"))
        sorted_expected_children = sorted(expected_children,
...
def test_compute_internal_counts(datasets, d, levels):
    ...
    err_msg = error_prefix + \
        "Incorrect compute_internal_counts return value. " \
        "Got {}, expected {}".format(total_count, test_data["total_count"])

    assert total_count == test_data["total_count"], err_msg
...
def test_prune_tree(datasets, test_file):
    ...
    compare_trees(pruned_tree, test_data["tree"],
                  error_prefix, compare_counts=True)


@pytest.mark.parametrize("d,levels", TREE_TEST_PARAMS)
def test_compute_rectangles(datasets, d, levels):
    ''' Test code for computing rectangles '''
