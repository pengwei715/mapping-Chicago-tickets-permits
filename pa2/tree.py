...
# pylint: disable-msg= len-as-condition, protected-access

class Tree(object):
    '''
    Class for representing trees with labels and counts.
    '''
    ...
    def __print_r(self, prefix, last, kformat, vformat, maxdepth, verbose):
        ...
        if self.children is None:
            ...
            for i, st in enumerate(self.children):
                ...
                st.__print_r(
                    newprefix, newlast, kformat, vformat, maxdepth, verbose)
...
def create_st(relevant_rows, hierarchy, hierarchy_labels, level_label):
    ...
    if len(hierarchy) == 0:
        ...
        return Tree(
            level_label, count=np.asscalar(relevant_rows["count"].sum()))
