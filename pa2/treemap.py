...
def validate_tuple_param(p, name):
    '''
    Check parameters for rectangles.
    '''
    ...
    assert p[0] >= 0.0 and p[1] >= 0.0, \
        "Incorrect value for rectangle {}: ({}, {}) ".format(
            name, p[0], p[1]) + "(both values must be >= 0)"
...
class Rectangle:
    ...
    def __init__(self, origin, size, label, verbose_label):
        ...
        assert verbose_label is not None, \
            "Rectangle verbose_label can't be None"
        assert isinstance(
    ...
    def __str__(self):
        ...
        return "RECTANGLE {:.4f} {:.4f} {:.4f} {:.4f} {}".format(
            self.x, self.y, self.width, self.height, label)
...
def cmd(diversity_file, categories, prune, output):
    '''
    Do the work! See above for parameter descriptions.
    '''
